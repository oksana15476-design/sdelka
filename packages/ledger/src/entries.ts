...
/**
 * Реверс расчёта **целиком**: расчёт записан ошибочно и отматывается назад.
 *
 * ```
 * Кт client:{плательщик}:tranche:{сделка}:{транш}   брутто   файл транша
 *     Дт client:{получатель}:free                     нетто   файл получателя
 * Дт fee:receivable                                 комиссия   файл транша
 * Дт bank:nominal                                    брутто   файл транша
 *     Кт bank:nominal                                  нетто   файл получателя
 * Кт transit:fee                                    комиссия   файл транша
 * ```
 *
 * **Зачем это здесь, а не «соберите проводки руками».** Законная операция была
 * **невыразима**: запись собиралась, но `appendEntry` отвечал на неё
 * `journalFeeAccruedTwice` — потому что дебет требования по комиссии с
 * отнесением к траншу читался как **начисление**, а начисление по траншу уже
 * было. Механизм запрещал не то, что собирался: не второе начисление, а
 * возврат требования, снятого этим же расчётом. Проба и разбор — в
 * `test/settlement-reversal.test.ts`.
 *
 * Обратная запись строится **зеркалом самой записи расчёта**, а не пересчётом
 * сумм: суммы, счета и отнесения берутся из неё, у каждой проводки меняется
 * только сторона. Пересчёт был бы вторым местом, где живёт форма расчёта, — а
 * повторённая от руки форма расходится молча (тот же довод, что у
 * `refundToSourceAccount`). Отсюда и свойство, которое нечем нарушить: реверс
 * возвращает ровно то, что расчёт двинул, — ни тетри сверх.
 *
 * Объявление расчёта (`settles`) переносится в реверс как есть: движение
 * обязательства между владельцами без него невозможно
 * (`assertClientOwnerMoveOnlySettles`), а сочинять для отмены **другое**
 * объявление значило бы отматывать один расчёт под видом другого. Потолок
 * удержания реверс не мерит и мерить не может: запертая часть здесь
 * кредитуется, разница брутто и нетто выходит отрицательной, и правило
 * пропускает такую запись по знаку (см. `assertSettlementCeiling`).
 *
 * **Начисление комиссии этим не отменяется.** Требование по комиссии
 * возвращается открытым: доход признан, расчёт снят, комиссия снова «начислена
 * и не удержана». Если сделка уходит в возвратную ветвь — начисление снимается
 * своей записью (`reverseFeeAccrual`, §4.4), и это два разных факта, а не один.
 */
export function reverseTrancheSettlement(meta: EntryMeta, settlement: JournalEntry): JournalEntry {
  const settles = settlement.settles;
  if (settlement.kind !== 'settlement' || settles === null) {
    // Отматывать нечего: либо это не расчёт, либо это уже исправление, и
    // «исправление исправления» — отдельная операция с отдельной ссылкой.
    throw new LedgerError(LedgerErrorCode.entryReversalTargetNotSettlement, {
      id: settlement.id,
      kind: settlement.kind,
      settles: settles === null ? 'null' : `${settles.deal.dealId}:${settles.deal.trancheId}`,
    });
  }
  return createJournalEntry({
    ...meta,
    ...
    settles,
    postings: settlement.postings.map((posting) =>
      posting.direction === 'debit'
        ? credit(posting.account, posting.amount, posting.attribution)
        : debit(posting.account, posting.amount, posting.attribution),
    ),
  });
}
