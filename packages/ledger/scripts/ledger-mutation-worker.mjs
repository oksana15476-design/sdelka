#!/usr/bin/env node
/**
 * Рабочий процесс мутационного прогона учёта: один живой vitest, много
 * мутаций.
 *
 * ## Зачем не «процесс на мутацию»
 *
 * Полный набор пакета — девятнадцать файлов и двести тестов; холодный запуск
 * `vitest run` стоит около четырёх секунд, из которых почти всё уходит на
 * разбор одних и тех же исходников заново. На тысяче с лишним мутаций это часы,
 * а проверка, которую нельзя прогнать дважды за рабочий день, перестаёт быть
 ...
 * любом заголовке цикла со счётчиком). Синхронный вечный цикл в рабочем потоке не
 ...
 * рабочий процесс отделён от надзирателя (`ledger-mutation.mjs`), который
...
import { MUTATION_ENV, findMutant, targetFile } from './ledger-mutants.mjs';
