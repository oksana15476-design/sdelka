#!/usr/bin/env node
/**
 * Мутационный прогон пакета приёма.
 *
 * ## Зачем
 *
 * Набор тестов, который не падает после порчи исходника, ничего не проверяет —
 * он только подтверждает, что функция вернула значение. Проверить это можно
 * единственным способом: по очереди испортить каждую величину, каждый вызов и
 * каждый ключ причины и посмотреть, заметит ли набор. Здесь это команда, которая
 * заканчивается ненулевым кодом, а не намерение провести проверку однажды.
 *
 * Классы мутаций и их обоснование — в `intake-mutants.mjs`.
 *
 * ## Три исхода, а не два
 *
 ...
 *    набором, каталог рассинхронизирован с текстом). Отдельный исход обязателен:
 *    без него сломанная подмена читается как «выжил», то есть как отсутствие
 *    тестов, и уводит работу не туда.
 *
 * ## Дерево не меняется
 *
 ...
 * ## Как запускать
 *
 *     pnpm --filter @sdelka/intake mutation
 *     pnpm --filter @sdelka/intake mutation -- --operator=key --file=disclosure.ts
 *     pnpm --filter @sdelka/intake mutation -- --list
 */
...
function parseArguments(argv) {
  ...
  for (const argument of argv) {
    ...
    if (match === null) throw new Error(`intake.mutation.bad_argument:${argument}`);
    ...
    else throw new Error(`intake.mutation.bad_argument:${argument}`);
  }
  return options;
}

/**
 * Отпечаток пакета: путь и содержимое каждого файла `packages/intake`, который
 * git считает частью проекта. Через git, а не обходом каталогов: так в отпечаток
 * не попадают `node_modules`, а попадает всё, что попадёт в коммит.
 *
 ...
 * мутируется только `packages/intake/src`. Отпечаток всего дерева отвечал бы на
 * другой вопрос — «менял ли кто-нибудь что-нибудь в репозитории за эти минуты»,
 * — и в дереве, где параллельно работают другие руки, отвечал бы «да» всегда.
 * Проверка, которая падает по чужой причине, перестаёт что-либо значить уже на
 * второй раз.
 */
function treeFingerprint() {
  const listed = (args) =>
    spawnSync('git', args, { cwd: REPO_ROOT, encoding: 'utf8', maxBuffer: 64 * 1024 * 1024 });
  const tracked = listed(['ls-files', '-z', '--', 'packages/intake']);
  const untracked = listed(['ls-files', '--others', '--exclude-standard', '-z', '--', 'packages/intake']);
  if (tracked.status !== 0 || untracked.status !== 0) throw new Error('intake.mutation.git_failed');
 ...
 * цикл (мутация `index -= 1` → `index -= 0`) не прерывается изнутри процесса:
 * событийный цикл занят, таймаут теста не сработает. Поэтому надзиратель ждёт
 * ответа не дольше `RUN_TIMEOUT_MS`, убивает процесс и поднимает новый, а
 * мутацию засчитывает убитой — поведение изменилось так, что прогон не
 * завершился вовсе.
 */
...
function startWorker(onLine, onExit) {
  const child = spawn('node', ['scripts/intake-mutation-worker.mjs'], {
    cwd: PACKAGE_ROOT,
    env: { ...process.env },
