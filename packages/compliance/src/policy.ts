...
/**
 * ⚠ **На боевом пути эти границы приходят версией настройки, а не константой.**
 *
 * Величина стала версионируемой (`@sdelka/limits`, домен `queue_age`) и
 * разрешается **на момент наблюдения**: границы не прилипают намеренно —
 * ужесточение норматива обязано подействовать на уже стоящие задачи, включая ту,
 * из-за которой оно принято. Подставляет её `compliancePolicyAt`; значение,
 * стоящее в `POLICY_2026_09_03.queue`, существует затем, чтобы им завели первую
 * версию журнала, и совпадение двух мест закреплено тестом
 * (`packages/limits/test/applied.test.ts`).
 *
 * Обратной зависимости здесь нет и быть не может: `limits → settings → auth →
 * compliance`, и ребро назад замкнуло бы цикл. Комплаенс получает величину
 * аргументом и о настройках не знает.
 */
