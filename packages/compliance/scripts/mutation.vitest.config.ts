...
// типов, потому что его вторая половина (`compliance-mutation.mjs`) запускается
...
/**
 * Конфигурация мутационного прогона комплаенса:
 ...
 */
function complianceMutation(): Plugin {
  return {
    name: 'sdelka:compliance-mutation-source',
    enforce: 'pre',
...
export default defineConfig({
  ...
  test: {
     ...
     * стоил бы полного прогона всех двадцати одного набора, и тысяча мутаций
     * заняла бы часы — то есть проверку перестали бы запускать.
     *
     ...
     *
     * Пакет чистый: ни один тест не пишет в глобальное состояние, все константы
     * заморожены, сети и файлов нет. Изоляция здесь покупает не корректность, а
     * двадцать один запуск окружения на каждую из девяти сотен мутаций.
     *
