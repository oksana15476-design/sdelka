#!/usr/bin/env node
/**
 * Рабочий процесс мутационного прогона комплаенса: один живой vitest, много
 * мутаций.
 *
 * ## Зачем не «процесс на мутацию»
 *
 * Полный набор пакета — двадцать один файл и четыреста с лишним тестов; холодный
 * запуск `vitest run` стоит около десяти секунд, из которых почти всё уходит на
 * разбор одних и тех же исходников заново. На тысяче мутаций это часы, а
 * проверка, которую нельзя прогнать дважды за рабочий день, перестаёт быть
 * проверкой: её перестают запускать.
 *
 ...
 * одного — мутируемого: он помечается изменённым, и только он собирается заново.
 *
 * ## Почему процессов несколько и ими управляет надзиратель
 *
 * Мутация числа умеет увести код в вечный цикл (`index += 1` → `index += 0` в
 * разборе графем `names.ts`). Синхронный вечный цикл в рабочем потоке не
 * прерывается ни таймаутом теста, ни чем-либо внутри процесса: событийный цикл
 * занят. Единственный способ его прекратить — убить процесс снаружи. Поэтому
 * рабочий процесс отделён от надзирателя (`compliance-mutation.mjs`), который
 * его и убивает, засчитывая зацикливание как убитого мутанта: поведение
 * изменилось так, что прогон не дошёл до конца.
 *
 * ## Протокол
 *
...
import { MUTATION_ENV, findMutant, targetFile } from './compliance-mutants.mjs';
...
async function runMutant(id) {
  ...
  // Порядок проверок — не вкусовщина. Сначала «подмена дошла»: если подмена
  // сломала сборку модуля, наборы тоже упадут, и без этой проверки первой
  // сломанная проверка засчиталась бы как «правило проверяется».
  if (!applied) return ['broken', 'подмена не дошла до исходника'];
  if (failed > 0) return ['killed', ''];
  // Наборы прогнаны не все, но и не упал ни один: это не остановка по падению
  // (`bail`), а обрыв прогона — вердикту верить нельзя.
