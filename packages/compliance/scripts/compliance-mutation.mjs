#!/usr/bin/env node
/**
 * Мутационный прогон пакета комплаенса.
 *
 * ## Зачем
 *
 * Комплаенс — это пакет **политических констант**: пороги скрининга, веса
 * ансамбля имён, доли концентрации, ступени эскалации очереди, коды причин, по
 * которым оператор принимает решение. Ни у одного из этих значений нет второго
 * места, где расхождение стало бы видно: порог, сдвинутый на единицу, не ломает
 * ни один сценарий — он молча меняет, кого пропустят. Набор тестов, который не
 * падает после такого сдвига, ничего про порог не проверяет; он проверяет, что
 * функция вернула значение.
 *
 * Проверить это можно единственным способом: по очереди испортить каждую
 * величину, каждую границу сравнения и каждый ключ причины и посмотреть, заметит
 * ли набор. Здесь это команда, которая заканчивается ненулевым кодом, а не
 * намерение провести проверку однажды.
 *
 * Классы мутаций и их обоснование — в `compliance-mutants.mjs`.
 *
 * ## Три исхода, а не два
 *
 ...
 * ## Как запускать
 *
 *     pnpm --filter @sdelka/compliance mutation
 *     pnpm --filter @sdelka/compliance mutation -- --operator=key --file=screening.ts
 *     pnpm --filter @sdelka/compliance mutation -- --file=detectors/payer.ts
 *     pnpm --filter @sdelka/compliance mutation -- --list
 */
...
function parseArguments(argv) {
  ...
  for (const argument of argv) {
    ...
    if (match === null) throw new Error(`compliance.mutation.bad_argument:${argument}`);
    ...
    else throw new Error(`compliance.mutation.bad_argument:${argument}`);
  }
  return options;
}

/**
 * Отпечаток пакета: путь и содержимое каждого файла `packages/compliance`,
 ...
 * мутируется только `packages/compliance/src`. Отпечаток всего дерева отвечал бы
...
function treeFingerprint() {
  ...
  const tracked = listed(['ls-files', '-z', '--', 'packages/compliance']);
  ...
  if (tracked.status !== 0 || untracked.status !== 0) {
    throw new Error('compliance.mutation.git_failed');
  }
...
function startWorker(onLine, onExit) {
  const child = spawn('node', ['scripts/compliance-mutation-worker.mjs'], {
    cwd: PACKAGE_ROOT,
    env: { ...process.env },
