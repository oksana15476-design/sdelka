...
export function projectSettlementIntent(
  ...
  const settlement = settleTrancheToClientAccount(
    ...
    trancheSettlement(
      ...
      intent.attestation,
      // Потолок удержания транша — из намерения, а не из настройки приложения.
      // Приложение здесь снова ничего не решает: политику назвал домен в момент
      // решения (`CORE.md` Ф11), а расширить предел она не может — на записи
      // действует строжайший из объявленного и жёсткого предела учёта.
      // Пока эта строка отсутствовала, тариф в 1,5 % проходил по траншу,
      // которому владелец назначил один процент: запись мерилась чужим, более
      // широким пределом (`packages/e2e`, `fee-ceiling-policy.test.ts`).
      intent.feeCeiling,
    ),
    amount,
    accrual,
