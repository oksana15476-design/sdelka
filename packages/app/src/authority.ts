...
/**
 * Положить в мир сессию, поднятую из хранилища.
 *
 * ## Зачем это отдельная дверь
 *
 * `openSession` выдаёт сессию **сейчас**: политика роли смотрит на предъявленные
 * подтверждения и на запрошенный срок. Сессия, пережившая перезапуск процесса,
 * выдана раньше и уже проверена — переспрашивать политику по ней нельзя: срок
 * посчитался бы от нового момента, то есть перезапуск продлевал бы сессии.
 *
 * ## Почему это не дыра
 *
 * Полномочий функция не даёт ни одного: `authorize` заново спрашивает
 * `decideCapability`, и истёкшая, простоявшая или отозванная сессия получит
 * отказ ровно так же, как получила бы, не покидая память. Собрать `Session`
 * литералом и так может любой — тип открыт, а `openSession` уже принимает роль
 * аргументом; хранилищем ограничивается не тип, а то, откуда значение берётся.
 *
 * Ровно поэтому вызывающий здесь один — транспорт, читающий сессию по
 * идентификатору из куки (`@sdelka/db`, `loadSession`), и ни один шаг мира.
 */
export function adoptSession(world: World, session: Session): World {
  return withSession(world, session);
}
