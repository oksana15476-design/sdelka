 ...
 *
 * Прежде это был отказ **на обычном пути**: `manage_settings` есть ровно у
 * `principal`, а `principal` в `AUDIT_ROLES` не переезжал никуда, и любая версия
 * настройки упиралась сюда. Пробел закрыт миграцией `0023`
 * (`sdelka.audit_role`), и успешная ветвь стала достижимой — E16-12 разблокирован.
 *
 * Отказ при этом остаётся, и не про запас: роль, заведённая без метки журнала,
 * обязана останавливать запись. Подставить похожую нельзя — `principal`,
 * записанный как `operator`, останется в вечном журнале ложью о том, кто двигал
 * деньги (красная линия №11).
 */
