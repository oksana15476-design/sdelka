 ...
 *
 * Куда событие ложится — сказано в `journal.ts`: `session_established`,
 * `session_denied` и `role_changed` в `AUDIT_RECORD_KINDS` теперь есть. Шести
 * видам событий (отзыв сессии, подтверждение второго фактора, дежурство, итог
 * авторизации) вида записи по-прежнему нет; они перечислены
 * `unjournaledAuthEventKinds`, а не потеряны молчанием. Класть их под
 * `decision_made` нельзя: у того тела обязательны версия политики и непустой
 * пакет доказательств, и подставлять их ради формы значит врать журналу.
 *
