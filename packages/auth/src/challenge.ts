...
export interface IdentityChallenge {
  ...
  /**
   * Когда код по этому вызову **последний раз** ушёл в канал. `null` — не
   * уходил ни разу: канал отказал на выдаче, и человеку не сообщили ничем.
   *
   * Поле здесь потому, что состояние потока запросов «пришлите код» держать
   * больше негде: своей таблицы у него нет и не заводится
   * (`DECISIONS-REVIEW.md` §Z2, вариант Б — «состояние уже в базе»), а колонка
   * `delivered_at` в `identity_challenge` существует с `0024`. Правило, которое
   * по нему считается, живёт в `code-request.ts`.
   *
   * «Последний раз», а не «первый»: отметка двигается на каждую отправку,
   * включая повторную (`0025`). Неподвижная отметка сделала бы окно повторной
   * отправки бесконечным — не «правило не работает», а «работает неверно».
   *
   * Обязательное, а не необязательное: хранилище, забывшее поднять отметку,
   * иначе молча получало бы умолчание, и ограничение считалось бы по
   * придуманному моменту. Пусть лучше не соберётся.
   */
...
export function issueChallenge(
  ...
  return Object.freeze({
    ...
    consumedAt: null,
    // Выданный вызов ещё не доставлен: доставка идёт после записи и может
    // отказать. `null` здесь — не «неизвестно», а «в канал не уходило ни разу».
    deliveredAt: null,
  });
}

/**
 * Отметка об отправке — **новое значение вызова, а не побочное действие**.
 *
 * Ставится на каждую отправку, включая повторную: отметка, которая ставится
 * один раз, отвечает на вопрос «доставляли ли вообще», а окно повторной
 * отправки спрашивает «когда доставляли **последний** раз». Разница не
 * теоретическая — по неподвижной отметке окно открывается один раз и больше не
 * закрывается никогда (`code-request.ts`).
 */
export function recordDelivery(challenge: IdentityChallenge, at: Instant): IdentityChallenge {
  return Object.freeze({ ...challenge, deliveredAt: at });
}
