 ...
 * Ещё пять — операционная механика расчёта, `ACTORS.md` §5.1.1: до них семь
 * шагов приложения выполнялись под чужим полномочием (`create_deal`), потому
 * что своего у них не было вовсе. Итого 33.
 */
export const CAPABILITIES = [
  ...
  /*
   * --- Операционная механика расчёта (`ACTORS.md` §5.1.1) ---
   *
   * **Дописываются только в конец, и это не стиль.** Перечень зеркалится
   * меткой перечня `sdelka.capability` (`packages/db`), а `ALTER TYPE … ADD
   * VALUE` иначе не умеет; порядок меток виден в `ORDER BY`, и тест дрейфа
   * сверяет его построчно. Группировка по смыслу поэтому кончается здесь:
   * дальше идёт хронология, и она честнее переставленных строк.
   *
   * Разведены по смыслу, а не собраны в одно «вести расчёт»: одно полномочие
   * склеило бы того, кто **готовит** операцию, с тем, кто **вносит внешний
   * факт** о ней, и с тем, кто **двигает деньги платформы**.
   */
  'prepare_settlement',
  'record_bank_outcome',
  'operate_treasury',
...
export const CAPABILITY_SPECS: Readonly<Record<Capability, CapabilitySpec>> = Object.freeze({
  ...
  /**
   * Подготовка расчёта: инструкции на оплату, отнесение поступления на транш,
   * запрос резерва и возврата, отметка резервирования и расчёта на уровне
   * сделки, заявка на разбор отката.
   *
   * Второго фактора нет — единственное из пяти без него, и это не послабление:
   * шаг ничего не двигает сам, а всё, что он готовит, проходит через
   * утверждение, у которого фактор есть. Требовать подтверждение на выдаче
   * инструкций значит требовать его двадцать раз в день там, где цена ошибки —
   * переделанная бумага.
   */
  prepare_settlement: {
    effect: 'prepare',
    secondFactor: 'none',
    journaled: true,
    source: 'ACTORS.md §5.1.1 (Ф4, Ф6, Ф8, Ф9)',
  },
  /**
   * Внесение внешнего факта платежа: исход провайдера, зачисление по выписке,
   * удержание и возврат платежа третьего лица, приход списанного, закрытие
   * сверки.
   *
   * `prepare`, а не `release`: шаг **записывает то, что сделал банк**, и сам
   * ничего не разрешает — разрешение стоит раньше, на `approve_payout`. Второй
   * фактор обязателен: подменённый исход провайдера — это «выплата прошла» там,
   * где деньги не ушли (красная линия №8), и разбирается он через сверку.
   */
  record_bank_outcome: {
    effect: 'prepare',
    secondFactor: 'step_up',
    journaled: true,
    source: 'ACTORS.md §5.1.1 (Ф4, Ф8, Ф10)',
  },
  /**
   * Казначейские операции: три момента конвертации, признание и довнесение
   * недостачи корреспондента, получение комиссии на операционный счёт.
   *
   * `release`, а не `prepare`, — единственное из пяти: здесь двигаются **деньги
   * платформы** (недостачу признаёт расходом платформа, комиссия уходит на
   * операционный счёт, спред закрывается поставкой), а не готовится чужая
   * операция. Отсюда и Н6 в разборе несовместимостей: роль, видящая маржу, ни
   * одного действия этого класса не совершает.
   */
  operate_treasury: {
    effect: 'release',
    secondFactor: 'step_up',
    journaled: true,
    source: 'ACTORS.md §5.1.1 (Ф5, Ф10, Ф16)',
  },
  /**
   * Заявка на вывод остатка клиента: завести, отправить в банк после подписей,
   * отменить. Подписи собираются отдельно (`approve_payout`, кворум §5.2), и
   * своя же подпись заявителя не считается — это guard домена, а не право.
   */
  conduct_withdrawal: {
    effect: 'prepare',
    secondFactor: 'step_up',
    journaled: true,
    source: 'ACTORS.md §5.1.1 (Ф14)',
  },
  /**
   * Правка фактов транша мимо автомата.
   *
   * ⚠ **Чёрный ход, а не функция продукта** — `ACTORS.md` §5.1.1 п.4. Правильный
   * ответ — убрать его вовсе, заменив сценарии на события; полномочие, выданное
   * никому (как `manage_access`), сегодня невозможно: на чёрном ходе стоят
   * сценарии guard'ов. Пока он есть, у него есть имя, второй фактор и запись в
   * журнал — то есть он виден, а не растворён в «оператор ведёт сделку».
   */
  patch_tranche_facts: {
    effect: 'prepare',
    secondFactor: 'step_up',
    journaled: true,
    source: 'ACTORS.md §5.1.1 (чёрный ход, введён реализацией)',
  },
});
