...
export function separationRulesFor(capability: Capability): readonly SodRuleId[] {
  switch (capability) {
     ...
     *
     * Ни одной несовместимости **на фактах** здесь нет, и это ответ, а не
     * умолчание. Довод один на четыре: шаги идут в том числе с предметом
     * «платформа» (движение по невыясненным поступлениям, конвертация остатка
     * клиента), у которого фактов нет по построению, — полномочие, связанное
     * Н1, Н2, Н4 или Н5, с таким предметом не выдалось бы **вовсе**, то есть
     * зачислить деньги по выписке стало бы невозможно. Разводит эти шаги не
     * несовместимость, а сам перечень: у носителя `prepare_settlement` и
     * `record_bank_outcome` (ОП) нет ни одного полномочия класса «утверждение»,
     * а Н1 стоит на `approve_payout` и считается по тому же следу.
     */
    case 'prepare_settlement':
    case 'record_bank_outcome':
    case 'conduct_withdrawal':
    case 'patch_tranche_facts':
      return EMPTY;

    /*
     * Казначейство двигает деньги платформы — класс `release`, и Н6 его
     * связывает наравне со снятием блокировки: роль, видящая маржу, ни одной
     * такой операции не совершает. На фактах несовместимостей нет по доводу
     * выше — предмет казначейской операции чаще всего «платформа».
     */
    case 'operate_treasury':
      return N6;
