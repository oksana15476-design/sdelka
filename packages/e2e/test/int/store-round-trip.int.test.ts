 ...
 *
 * ⚠ **Перезапуск процесса этим НЕ чинится, и помечено это [открыто].** `seq`
 * поднятого мира взять неоткуда: `restoreWorld` отдаёт `RestoredWorld`, а не
 * `World`, — продолжить поднятый мир нечем вовсе. Пока такого продолжения нет,
 * повторный прогон начинает счёт заново и встречает свои же строки; повтором
 * они опознаются только потому, что шаги повторяются дословно (см. ниже про
 * `written + repeated`).
 *
 * ⚠ **`readJournal` области видимости не имеет вовсе** — ни у `pgWorldStore`,
 * ни у реализации в памяти. Поэтому поднятый журнал содержит записи **всех**
 * миров базы, и сравнение с миром идёт по его области имён (цепочке в
 * идентификаторе), а не целиком. Помечено **[открыто]**: чтение журнала по
 * области — это метод порта, которого нет.
 *
...
run(title, () => {
  ...
  it('мир поднимается из живой базы с теми же инвариантами', async () => {
     ...
     *
     * Отбор по цепочке — не ослабление, а следствие того, что `readJournal`
     * области видимости не имеет вовсе (заголовок файла, **[открыто]**): в
     * таблице живут и записи второго мира, и записи прошлых прогонов. Отбирать
     * стало чем ровно потому, что идентификатор теперь несёт цепочку
     * (`app/src/ids.ts`); до этого сравнение держалось на том, что второй мир
     * в базу не доходил.
     */
  ...
  it('второй мир в той же базе проходит и не сталкивается с первым', async () => {
    ...
    // Записи первого мира из общей таблицы никуда не делись.
    const all = new Set(restored.journal.entries.map((entry) => entry.id));
    expect([...known].every((id) => all.has(id))).toBe(true);
  });
});
