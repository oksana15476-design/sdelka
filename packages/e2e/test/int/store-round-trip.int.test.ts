 ...
 *
 * ⚠ **Основные сценарии набора идут по ОДНОМУ миру, и тесты продолжают друг
 * друга.** Так было заведено вынужденно: идентификатор записи журнала учёта был
 * счётчиком мира (`entry-${seq}-${label}`), счётчик начинается с нуля в каждом
 * новом мире, и **два разных мира сталкивались на `entry-<seq>-…`** — в памяти
 * у каждого теста своё хранилище, а живая база одна и переживает прогон.
 * Сегодня идентификатор несёт цепочку (`app/src/ids.ts`, `journalEntryId`), как
 * всегда нёс идентификатор записи журнала аудита, и второй мир в этой же базе
 * проходит целиком — это отдельный тест в конце файла. Порядок и продолжение
 * оставлены как есть: они проверяют и то, что счётчик мира сквозной.
 *
 * ⚠ **Перезапуск процесса чинится, но не этим набором.** Поднятый мир теперь
 * продолжается (`app/src/resume.ts`, `resumeWorld`), и счётчик записей берётся
 * из записанного. Здесь этим никто не пользуется: набор ведёт **один** мир от
 * первого шага до последнего, и продолжение поднятого проверяется отдельно
 * (`test/world-resume.test.ts`). Отсюда и форма утверждений ниже про
 * `written + repeated`: повторный прогон по-прежнему начинает мир заново.
 *
 * ⚠ **У чтения журнала теперь есть охват** (`app/src/store.ts`, `JournalScope`),
 * и подъём берёт его по своей цепочке. Поэтому поднятый журнал — записи
 * **своего** мира, а не всей базы; отбор по началу идентификатора в
 * утверждениях ниже остался как был и стал избыточным, а не необходимым.
 *
 ...
 * с записанным **весь** журнал — и делают это теперь написанным охватом
 * (`{ kind: 'everything' }`), потому что проверяют именно таблицу целиком.
 * Значит после этого набора те проверки красные, и лечится это чистой базой, а
 * не правкой здесь. Помечено **[открыто]**.
 *
 ...
 * покупатель здесь работает: столкновение на записи зачисления перестаёт быть
 * повтором и становится подменой, то есть именно тем, о чём хранилище обязано
 * сказать вслух, а не промолчать `repeated`.
 */
...
run(title, () => {
  ...
  it('мир поднимается из живой базы с теми же инвариантами', async () => {
     ...
     * заводилась `0021`. Сравнение полное **по значению**: `JournalEntry`
     * собирается на чтении тем же `createJournalEntry`, что и на записи,
     * поэтому потерянное поле видно здесь, а не в отчётности через месяц.
     *
     * Отбор по цепочке здесь **избыточен**: охват задаёт само чтение
     * (`JournalScope`), и поднятый журнал уже свой. Оставлен намеренно — он
     * ловил бы охват, отдавший лишнее, а стоит он ничего.
     */
    expect(restored.journal.entries.filter((entry) => entry.id.startsWith(`${CHAIN}:`))).toEqual(
  ...
  /**
   * Второй мир в той же базе. Тест, ради которого стоило гонять сценарий против
   * базы, а не против карты в памяти: пока он был красным, он назывался
   * «сталкивается на идентификаторе записи журнала».
   *
   * Дефект был в том, что идентификатор записи журнала учёта — счётчик мира, а
   * счётчик начинается с нуля в каждом новом мире: второй мир падал на
   * `db.step.conflict` по `ledger_entry`, не дойдя до первой денежной записи.
   * Идентификатор теперь несёт цепочку (`app/src/ids.ts`).
   *
   * **Стоит последним и не двигает курсор.** Мир здесь свой, продолжать им
   * нечего; а записи, которые он оставляет в общей таблице, попали бы в
   * `readJournal` теста «мир поднимается из живой базы» — у чтения журнала
   * области видимости нет вовсе (заголовок файла).
   */
  it('второй мир в той же базе проходит и не сталкивается с первым', async () => {
    const first = at().world;
    const other = await collectedPath(store(), OTHER_WORLD, null);

    expect(trancheStatusOf(other.world, OTHER_WORLD.trancheId)).toBe('collected');
    // `funding`, а не `funded`: путь второго мира кончается на собранных
    // деньгах, резерв траншей — уже следующий шаг.
    expect(dealStatusOf(other.world, OTHER_WORLD.dealId)).toBe('funding');
    // Строки второго мира действительно легли (или опознались повтором на
    // повторном прогоне), а не «прошли», ничего не записав.
    expect(laid(other)).toBeGreaterThan(0);

    /*
     * Область имён — цепочка аудита. Ни один идентификатор второго мира не
     * встречается у первого: без этого утверждения тест прошёл бы и на
     * совпавших дословно записях, которые база опознала бы повтором.
     */
    const mine = other.world.journal.entries.map((entry) => entry.id);
    expect(mine.length).toBeGreaterThan(0);
    expect(mine.every((id) => id.startsWith(`${OTHER_WORLD.chainId}:`))).toBe(true);
    const known = new Set(first.journal.entries.map((entry) => entry.id));
    expect(mine.filter((id) => known.has(id))).toEqual([]);

    /*
     * И то же самое со стороны базы: в общей таблице лежат записи обоих миров,
     * а своя область поднимается целиком и по значению.
     */
    const restored = await restoreWorld(store(), {
    ...
    expect(restoredViolations(restored)).toEqual([]);
    expect(
      restored.journal.entries.filter((entry) => entry.id.startsWith(`${OTHER_WORLD.chainId}:`)),
    ).toEqual(other.world.journal.entries);
    expect(restored.chain).toEqual(other.world.chain);
    /*
     * Записи первого мира из общей таблицы никуда не делись — но в охват
     * второго мира они больше не попадают. Отбор — это вопрос «кому записи», а
     * не потеря записей, и проверяется он подъёмом первого мира его же
     * охватом.
     */
    const all = new Set(restored.journal.entries.map((entry) => entry.id));
    expect([...known].some((id) => all.has(id))).toBe(false);
