 ...
 * тому же миру — и потому, что так короче, и потому, что продолжение мира само
 * по себе проверяется: счётчик записей у мира один на журнал учёта и журнал
 * аудита.
 *
 * Раньше это было не удобством, а необходимостью: идентификатор записи журнала
 * учёта был счётчиком мира (`entry-<seq>-<label>`) и области имён не имел
 * вовсе, поэтому два разных мира в одной базе сталкивались на `entry-1-…`.
 * Сегодня идентификатор несёт цепочку (`app/src/ids.ts`), и два мира в одной
 * базе живут; что именно живут — проверяется тестом «два мира в одной базе»
 * в `store-round-trip.test.ts` и в `test/int/store-round-trip.int.test.ts`.
 */
