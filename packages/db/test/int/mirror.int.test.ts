  ...
  for (const scenario of scenarios()) {
    it(scenario.name, async () => {
      ...
      const actual = await withRollback(pool, async (client) => {
        ...
        // Проверки отложены до коммита; здесь коммита нет и не будет, поэтому
        // проверить их надо явно — иначе сценарий-нарушитель упал бы на
        // откате, а не показал бы содержимое сводки.
