...
export interface PgStoreOptions {
   ...
   *
   * **Прежде под этой ролью не дописывался журнал аудита.**
   ...
   * инвариант 21 у роли приложения отбирает: `INSERT` был разрешён грантом и
   * невозможен на практике. `0020_audit_append_only.sql` перевела функцию на
   * `SECURITY DEFINER` — блокировку берёт владелец схемы, права роли приложения
   * не изменились. Что роль по-прежнему не может ни изменить, ни удалить, ни
   * опустошить журналы, проверяется живыми попытками в
   * `test/int/store-grants.int.test.ts`, а не только списком грантов: находка
   * ровно тем и была, что список грантов и то, что роль может на самом деле, —
   * разные вещи.
   */
