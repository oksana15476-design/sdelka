...
export interface PgStoreOptions {
   ...
   *
   * ⚠ **[открыто] Под ролью приложения журнал аудита сегодня не дописывается.**
   * `sdelka.assert_audit_chain()` объявлена без `SECURITY DEFINER` и берёт
   * `SELECT … FOR UPDATE`, а `FOR UPDATE` требует права `UPDATE`, которое
   * инвариант 21 у роли приложения отбирает. `INSERT` разрешён грантом и
   * невозможен на практике; проба и разбор — в
   * `test/int/store-grants.int.test.ts`. До правки миграции шаг, дописывающий
   * журнал аудита, приходится вести под ролью владельца — то есть с правами,
   * которых у приложения быть не должно. Умолчание здесь **не** меняется на
   * владельца намеренно: тихо расширить права значило бы закрыть находку.
   */
