...
export const PENDING_NATIVE_REVIEW_KEYS: readonly string[] = Object.freeze([
  ...
  /*
   * Заявка покупателя на сделку как настоящая запись (`ROAD-TO-SCREEN.md` §3.2 и
   * §3.4). Двадцать одна строка: пустой кабинет, поданная заявка, четыре
   * состояния заявки, остановленный приём, отказ подачи и повторная отправка.
   *
   * Русский **принят главредом** (2026-09-09); английский и грузинский собраны
   * терминологией, уже отгруженной в словарях (`განაცხადი` — заявка, `გარიგება`
   * — сделка, `ოპერატორი`, `რიგში`, `განიხილება`, en `request` / `deal` / `the
   * other party`), и проверены только машиной — длиной на грузинском и
   * запрещённой лексикой по трём языкам.
   *
   * Что главред переписал и почему — по одной причине на строку:
   *
   * · **`error.body`** говорил «поправьте данные и отправьте снова». Отказ,
   *   который сюда приходит, — общий (§3.3 правило 2), и **дела в полях он не
   *   устанавливает**: человек шёл перечитывать кадастровый код, с которым всё в
   *   порядке. Побуждение осталось, утверждение снято: «можно проверить данные и
   *   отправить снова».
   * · **`error.why`** обещал, что точная причина у нас есть и не называется ради
   *   чужих данных. У части отказов её нет вовсе, и строка утверждала за них
   *   мотив. Теперь названо правило, верное всегда: ответ один на все случаи,
   *   потому что по разнице в ответах узнают о чужих заявках и сделках. Строка
   *   осталась: молчание здесь читается как «сломалось», и тот же приём уже
   *   принят на входе (`signIn.sent.body`).
   * · **`intakeHalted.title`** стоял «Приём новых сделок остановлен» — почти
   *   дословно `banner.coverage.title`, и на экране с баннером человек прочитал
   *   бы одно дважды, а своё («заявка принята») — во второй строке. Заголовок
   *   говорит теперь о заявке, общий факт ушёл в тело.
   * · **`status.accepted.badge`** — «В работе» → «В разборе». Чип стоит рядом с
   *   карточками сделок, где «в работе» означает **сделку**; «разбор» — слово,
   *   которым оператор уже назван в словаре (`withdraw.state.blocked.body`), и
   *   он же стоит в `status.accepted.meaning`.
   *
   * Принято без правки: `error.title` («Заявка не отправлена»). «Не принята»
   * точнее по механике, но читается как решение оператора, а это соседнее
   * состояние `status.declined`.
   *
   * Носителю отдельно:
   *
   * · **`status.accepted.badge`** — «В разборе» переведено `განიხილება`
   *   («рассматривается»), тем же корнем, что `განიხილავს` в
   *   `withdraw.state.blocked.body`. Проверить, что чип не читается как
   *   состояние **сделки**: рядом с ним стоит `გარიგება შექმნილია`.
   * · **`intakeHalted.title`** — `განაცხადი მიღებულია, გარიგება მოგვიანებით`:
   *   вторая часть эллиптична, глагол опущен ради бюджета. Проверить, что она
   *   звучит по-грузински, а не обрывком. Прямой перевод русского («приём»
   *   рядом с «принята») давал в грузинском один корень `მიღება` в двух
   *   значениях, и потому его здесь нет.
   * · **`status.converted.meaning`** несёт `გარიგების ბარათში`. Слово `ბარათი` в
   *   словаре уже значит и «карточку сделки» (`assurance.whereMoney.onRevoked`),
   *   и «банковскую карту» (`landing.money.card`). Развести термины — работа по
   *   всему словарю, здесь она не делалась.
   * · **`error.why`** — смысл (одна причина на все отказы, различимые снаружи)
   *   обязан сохраниться дословно: это правило спеки, а не оборот речи.
   *
   * Бюджеты длины, под которые фразы подрезаны (`verify-ui.mjs`): заголовок 44
   * знака на грузинском — `intakeHalted.title` 41, `empty.title` 37; чип 32 —
   * самый длинный `გარიგება შექმნილია` 18; тело 240 и не больше трёх
   * предложений — самое длинное `error.body` около 155.
   */
  'dealApplication.empty.title',
  'dealApplication.empty.body',
  'dealApplication.empty.cta',
  'dealApplication.submitted.title',
  'dealApplication.submitted.body',
  'dealApplication.status.submitted.badge',
  'dealApplication.status.submitted.meaning',
  'dealApplication.status.accepted.badge',
  'dealApplication.status.accepted.meaning',
  'dealApplication.status.declined.badge',
  'dealApplication.status.declined.meaning',
  'dealApplication.status.converted.badge',
  'dealApplication.status.converted.meaning',
  'dealApplication.intakeHalted.title',
  'dealApplication.intakeHalted.body',
  'dealApplication.error.title',
  'dealApplication.error.body',
  'dealApplication.error.why',
  'dealApplication.error.support',
  'dealApplication.repeat.title',
  'dealApplication.repeat.body',

  /*
   * Две строки формы заявки, переписанные ради той же дороги: они были приняты
   * раньше и обещали срок, которого продукт не держит.
   *
   * `newDeal.submit.note` говорил «Менеджер сделки свяжется в течение рабочего
   * дня». Публичного срока у нас нет: он изъят из целевых обещаний
   * (`DECISIONS-REVIEW.md` D4, **[открыто]** — цифру называет владелец), и D4
   * прямо велит не называть его нигде, пока она не названа. Второй экран той же
   * дороги срока не даёт вовсе, то есть два экрана обещали разное. Теперь оба
   * говорят одно, и оборот взят не новый, а уже принятый на публичной странице
   * (`landing.sent.body`): называем канал ответа и наши рабочие часы, а не срок.
   *
   * `newDeal.field.placeholder` звал того же человека «менеджером сделки», тогда
   * как весь остальной словарь — кабинет, консоль, вывод, заявка — зовёт его
   * оператором. Одно понятие, одно слово: «заполняет оператор».
   *
   * Правка задела en и ka, и до вычитки носителем они значатся здесь.
   */
  'newDeal.submit.note',
  'newDeal.field.placeholder',
]);
